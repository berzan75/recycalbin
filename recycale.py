...
def draw():
    ...
    if game_over:
        ...
        screen.draw.text("Try Again", fontsize=50, center=(CENTRE_X, CENTRE_Y + 30), color="black")
    if game_complete:
        screen.draw.text("Good Job", fontsize=50, center=CENTRE, color="black")
        screen.draw.text("Round 2?", fontsize=50, center=(CENTRE_X, CENTRE_Y + 30), color="black")
    else:
        for item in items:
            item.draw()
